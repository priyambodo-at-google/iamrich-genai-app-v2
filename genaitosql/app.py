...
with col1:
    st.subheader("")
    st.image("imgs/bqicon.png")
with col2:
    st.title(f"{config['application']['name']} ")

st.markdown(
"""
This App demonstrates the use of a Generative AI for answering questions over a SQL database.
""",
...
with st.sidebar:
    ...
    source_dataset_selected = st.selectbox("Select A Dataset", source_dataset_list)
    table_list = []
    if source_dataset_selected != "Please Select":
        table_list = ["Please Select"] + list_bq_tables_in_dataset(source_dataset_selected)
        table_selected = st.selectbox("Select Table", table_list)
    st.markdown("***")
...
with tab1:
    ...
    if source_dataset_selected == "Please Select":
        st.info("Please select a Dataset from the side panel")
    else:
        if table_selected == "Please Select":
            st.info("Please select a Table from the side panel")
        else:
